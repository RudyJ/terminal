  ...
  try {
    ...
    if (strcmp($method, 'help') == 0) {
      if (count($params) > 0) {
        ...
        $msg = "Comandos permitidos : \n  " . implode(", ", array_slice($methods, 0, -1)) . " and " .  $methods[count($methods)-1] . ".";
        $msg .= " \nDocumentação completa em http://wiki.hackerexperience.com \n ";
        echo response($msg, $id, null);
      }
    } else if (!in_array($method, $methods)) {
      $msg = 'Nâo existe o comando ' . $method;
